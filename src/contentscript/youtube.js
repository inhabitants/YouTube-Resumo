...
function addClassIfDarkMode(className) {
    if (isDarkMode()) {
        return className;
    }
...
function sanitizeWidget() {
    ...
    // Height Adjust
    document.querySelector("#yt_ai_summary_body").style.maxHeight = window.innerHeight - 380 + "px";
    document.querySelector("#yt_ai_summary_text").innerHTML = `
...
function createLangSelectBtns(langOptionsWithLink) {
    document.querySelector("#yt_ai_summary_lang_select").innerHTML = Array.from(langOptionsWithLink).map((langOption, index) => {
        return `<button class="yt_ai_summary_lang ${addClassIfDarkMode("yt_ai_summary_lang_dark")} ${(index == 0) ? "yt_ai_summary_lange_selected" : ""}" data-yt-transcript-lang="${langOption.language}">${langOption.language}</button>`;
    }).join("");
}
